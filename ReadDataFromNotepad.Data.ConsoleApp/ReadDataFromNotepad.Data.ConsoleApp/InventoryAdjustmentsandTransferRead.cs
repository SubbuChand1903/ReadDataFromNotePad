...
{
    class InventoryAdjustmentsandTransferRead
    {
        public DataTable ConvertToDataTable()
        {
            ...
            data.Columns.Add("MainLine", typeof(string)); DataRow dr = null;
            foreach (string st in stringarry)
            {
                if (!string.IsNullOrEmpty(st))
                {
                    ...
                    if (splitarray != null && splitarray.Length > 1)
                    {
                        ...
                        dr["Account1"] = splitarray[13];
                        dr["Location"] = splitarray[14];
                        dr["Subsidiary"] = splitarray[15];
                        dr["SubsidiaryNoHierarchy"] = splitarray[16];
                        dr["Status"] = splitarray[17];
