...
{
    class WorkOrderCompletionListRead
    {
            ...
            foreach (string st in stringarry)
            {
                if (!string.IsNullOrEmpty(st))
                {
                    ...
                    if (splitarray != null && splitarray.Length > 1)
                    {
                        ...
                        dr["Quantity"] = splitarray[11];
                        dr["Item"] = splitarray[12];
                        dr["CreatedFromType"] = splitarray[13];
                        dr["CreatedFromDocumentNumber"] = splitarray[14];
                        dr["CreatedFromInternalId"] = splitarray[15];
                        dr["Subsidiary"] = splitarray[16];
                        dr["SubsidiaryNoHierarchy"] = splitarray[17];
                        dr["InternalId"] = splitarray[18];
                        dr["MainLine"] = splitarray[19];
                        dr["LineId"] = splitarray[20];
                        data.Rows.Add(dr);
                    }
                }
            }
